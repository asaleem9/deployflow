...
app.conf.beat_schedule = {
    ...
    # Upstream telemetry disabled for DeployFlow — no instance metrics are
    # pushed to any external endpoint. The task also self-gates on the
    # instance's is_telemetry_enabled flag, which defaults off.
    ...
    # },
    # web3 on-chain integrations (DeployFlow)
    "web3-confirm-pending-deployments": {
        "task": "plane.web3.bgtasks.explorer_sync.confirm_pending_deployments",
        "schedule": crontab(minute="*/2"),  # every 2 minutes
    },
    "web3-sync-verification-status": {
        "task": "plane.web3.bgtasks.explorer_sync.sync_verification_status",
        "schedule": crontab(minute="*/10"),  # every 10 minutes
    },
    "web3-snapshot-gas-prices": {
        "task": "plane.web3.bgtasks.gas_snapshot.snapshot_gas_prices",
        "schedule": crontab(minute="*/10"),  # every 10 minutes
    },
    "web3-prune-gas-snapshots": {
        "task": "plane.web3.bgtasks.gas_snapshot.prune_gas_snapshots",
        "schedule": crontab(hour=3, minute=0),  # UTC 03:00
    },
    "web3-verify-bounty-payouts": {
        "task": "plane.web3.bgtasks.bounty_payout.verify_bounty_payouts",
